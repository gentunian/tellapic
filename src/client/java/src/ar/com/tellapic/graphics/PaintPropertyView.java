...
import ar.com.tellapic.utils.FontChooserComboBox;
import ar.com.tellapic.utils.Utils;

public class PaintPropertyView extends JPanel implements Observer {
	
	public static final String BEVEL_JOIN_ICON_PATH  = "/icons/tools/joinbevel.png";
	public static final String ROUND_JOIN_ICON_PATH  = "/icons/tools/joinround.png";
	public static final String MITER_JOIN_ICON_PATH  = "/icons/tools/joinmiter.png";
	public static final String ROUND_END_ICON_PATH   = "/icons/tools/capround.png";
	public static final String BUTT_END_ICON_PATH    = "/icons/tools/capbutt.png";
	public static final String SQUARE_END_ICON_PATH  = "/icons/tools/capsquare.png";
	public static final String[]  LINE_JOINS_ICON_PATHS = new String[] {
		MITER_JOIN_ICON_PATH,
		ROUND_JOIN_ICON_PATH,
		BEVEL_JOIN_ICON_PATH
	};
	public static final String[] END_CAPS_ICON_PATHS = new String[] {
		BUTT_END_ICON_PATH,
		ROUND_END_ICON_PATH,
...
//	private JComboBox                fontFaceCombo;
	private FontChooserComboBox      fontFaceCombo;
		...
		if (tool instanceof DrawingTool) {
			...
			fillColor.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent e) {
					IColorSelectorController c = new IColorSelectorController() {
						@Override
						public void handleColorChange(Color c) {
							controller.handleFillColorChange(c);
						}
					};
					ColorSelector selector = new ColorSelector(null, new Color(0,0,0,0), 0, 0, true, c);
			...
			popup.addSeparator();
			popup.add(symmetric);
			if (drawingTool.hasFillCapability())
				popup.add(fillColor);
			popup.addSeparator();
		}
		popup.add(tips);
	 ...
	 * 
	 */
	private void createOpacityOptions() {
...
//		fontFaceCombo  = new JComboBox(GraphicsEnvironment.getLocalGraphicsEnvironment().getAvailableFontFamilyNames());
		fontFaceCombo  = new FontChooserComboBox();
		fontSizeCombo  = new JComboBox(fontSizes);
		fontStyleCombo = new JComboBox(FontStyle.values());
...
//					textField.setFont(Font.decode("Droid-italic-10"));
		...
		/* Set font style combo properties */
		...
		fontStyleCombo.addMouseWheelListener(new ComboWheelListener(fontStyleCombo));
		
		/* Set fon size combo properties */
		...
		fontSizeCombo.addMouseWheelListener(new ComboWheelListener(fontSizeCombo));
		
		/* Set font face combo properties */
		fontFaceCombo.setPreferredSize(new Dimension(200, ToolView.ICON_SIZE + GAP));
...
//		fontFaceCombo.addMouseWheelListener(new ComboWheelListener(fontFaceCombo));
		fontFaceCombo.setPreviewFontSize(10);
	}
	...
	private void createStrokeOptions() {
		joinLabel    = new JLabel(Utils.msg.getString("strokejoin")+":");
		widthLabel   = new JLabel(Utils.msg.getString("strokewidth")+":");
//		opacityLabel = new JLabel(Utils.msg.getString("opacity")+":");
		capsLabel    = new JLabel(Utils.msg.getString("strokecaps")+":");
		dashLabel    = new JLabel(Utils.msg.getString("strokedash")+":");
		widthSpinner = new JSpinner(new SpinnerNumberModel(DEFAULT_WIDTH, 0.1, 70.0, 0.1));
//		opacitySpinner = new JSpinner(new SpinnerNumberModel(DEFAULT_OPACITY, 0.0, 100.0, 0.5));
		joinCombo    = new JComboBox(PaintPropertyStroke.LineJoinsType.values());
		capsCombo    = new JComboBox(PaintPropertyStroke.EndCapsType.values());
		dashCombo    = new JComboBox();

//		SpinnerTranslucentEditor opacityEditor = new SpinnerTranslucentEditor(opacitySpinner); 
		WidthSpinnerEditor       widthEditor   = new WidthSpinnerEditor(widthSpinner);
//		javax.swing.plaf.synth.SynthComboBoxUI.SynthComboBoxRenderer r;
...
//		opacitySpinner.addChangeListener(opacityEditor);
		...
		Utils.logMessage("rend: "+capsCombo.getRenderer());
...
//		opacityLabel.setFont(defaultTitleFont);
		...
		String[] array = new String[PaintPropertyStroke.EndCapsType.values().length];
		int i = 0;
		for (PaintPropertyStroke.EndCapsType ect : PaintPropertyStroke.EndCapsType.values())
			array[i++] = ect.toString();

		Dimension capsComboDimension = new Dimension(getMaxTextWidth(capsCombo, array) + gap, ToolView.ICON_SIZE + GAP);
		i = 0;
		for (PaintPropertyStroke.LineJoinsType ect : PaintPropertyStroke.LineJoinsType.values())
			array[i++] = ect.toString();
		
		Dimension joinComboDimension = new Dimension(getMaxTextWidth(joinCombo, array)+ gap, ToolView.ICON_SIZE + GAP);
...
//		ComboWheelListener   l4 = new ComboWheelListener(joinCombo);
		...
		capsCombo.addMouseWheelListener(new ComboWheelListener(capsCombo));
		joinCombo.addMouseWheelListener( new ComboWheelListener(joinCombo));
//		opacitySpinner.addMouseWheelListener(l1);
		widthSpinner.addMouseWheelListener(new SpinnerWheelListener(widthSpinner));
	}
	...
	private void manageColorChange(ColorSelector colorSelector, JComponent component) {
		...
		controller.handleStrokeColorChange(color);
	...
	public void showPanel(Tool tool) {
		...
		toolIcon.setIcon(new ImageIcon(Utils.createIconImage(ToolView.ICON_SIZE, ToolView.ICON_SIZE, tool.getIconPath())));
...
//		}
	...
	private void setStrokeProperties(PaintPropertyStroke strokeProperty) {
		EndCapsType   caps  = strokeProperty.getEndCaps();
		LineJoinsType join  = strokeProperty.getLineJoins();
		double        width = strokeProperty.getWidth();
		Color         color = strokeProperty.getColor();
	 ...
	 * @param colorProperty
	 */
	private void setStrokeColorProperty(PaintPropertyStroke strokeProperty) {
		colorField.setBackground(strokeProperty.getColor());
	}
	...
	private void setAlphaProperties(PaintPropertyAlpha opacityProperty) {
		double opacity = opacityProperty.getAlpha()*100;
...
//		opacitySpinner.setValue(DEFAULT_OPACITY);
		controller.handleStrokeColorChange(DEFAULT_COLOR);
...
//				if (tool instanceof DrawingTool && ((DrawingTool)tool).hasColorCapability())
//					setStrokeColorProperty(box.getStrokeProperty());
			}
		repaint();
		}
	}
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (action != null && controller != null) {
				...
				if (action.equals(SET_FONT_FACE_ACTION)) {
					FontChooserComboBox src = (FontChooserComboBox) arg0.getSource();
					controller.handleFontFaceChange(src.getSelectedFontName());
					textField.setFont(Font.decode((String)src.getSelectedFontName()+" 10"));
					...
					JComboBox src = (JComboBox) arg0.getSource();
					controller.handleFontStyleChange((FontStyle)src.getSelectedItem());
					
				} else if (action.equals(SET_CAPS_ACTION)){
					JComboBox src = (JComboBox) arg0.getSource();
					controller.handleEndCapsChange((EndCapsType) src.getSelectedItem());
					
				} else if (action.equals(SET_JOINS_ACTION)){
					JComboBox src = (JComboBox) arg0.getSource();
					controller.handleLineJoinsChange((LineJoinsType) src.getSelectedItem());
...
//					controller.handleZoomChange(1);
	 ...
	 *
	 */
	public static class LabelComboBoxRenderer extends DefaultListCellRenderer {
		private static final long serialVersionUID = 1L;
		private Map<Object, Icon> map; 
		public static final int ICON_GAP  = 3;
		
		public LabelComboBoxRenderer(Border b, String[] iconPaths, Object[] descriptions) {
			super();
			if (iconPaths.length != descriptions.length)
				throw new IllegalArgumentException("Icon paths array length should be the same length as descriptions array");
			
			map = new HashMap<Object, Icon>();
			for(int i = 0; i < iconPaths.length; i++) {
				Image image = Utils.createIconImage(ToolView.ICON_SIZE, ToolView.ICON_SIZE, iconPaths[i]);
				map.put(descriptions[i], new ImageIcon(image));
			}
		...
		public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
			...
			setIcon(map.get(value));
			setText(value.toString().replaceAll("_", " "));
...
//					popup.show(PaintPropertyView.this, opacitySpinner.getX(), opacitySpinner.getHeight());
	...
	private class ColorSelectionListener extends MouseAdapter {
		...
		IColorSelectorController cController;
		
		public ColorSelectionListener(JComponent c) {
			component     = c;
			cController = new IColorSelectorController() {
				@Override
				public void handleColorChange(Color c) {
